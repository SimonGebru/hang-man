...
const startGame = () => {
  ...
  if (hiddenWordDisplay && guessedLettersDisplay && resultText) {
    ...
    console.error(
      "Ett eller flera DOM-element saknas och kan inte uppdateras."
    );
  }
};
...
document.addEventListener("keypress", (event) => {
  ...
  if (
    alphabet.includes(chosenLetter) &&
    !guessedLetters.includes(chosenLetter)
  ) {
    handleGuess(chosenLetter);
...
function speakLetter(letter) {
  if (isSpeechEnabled) {
    // Kontrollera om talfunktionen är aktiverad
...
window.addEventListener("keydown", function (event) {
  ...
  if (isSpeechEnabled && letter.length === 1 && letter.match(/[a-zA-öÖ]/)) {
    // Kontrollera talstatus
    speakLetter(letter);
  }
});
...
document
  .getElementById("toggle-speech-button")
